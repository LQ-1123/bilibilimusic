...
- **漫游电台（A5）**：队列面板 ∞ 开关（或详情页「电台」按钮）打开后，队列见底自动以当前歌为种子从推荐池续歌（试听流，不落盘）；跳过电台歌＝出池负反馈。推荐池靠起播搭车采集维持，频控三重闸（同种子去重 / 全局 10 分钟 / 每日 20 次），零后台任务。
- **音质放开（A3）**：fnval 带 Hi-Res/杜比位，best/192/132/64 档位帽由后端消费；**修复 Hi-Res 永远拿不到的解析 bug**——`dash.flac` 是 `{display, audio}` 包装层，流在 `flac.audio` 之下，旧代码把包装层直接喂解析器导致无损被静默丢弃（恰好被「非会员不下发」掩盖，会员到期复充后真机暴露）；含两种响应形态的回归测试。
- **弱网自动降档（A6）**：Wi-Fi/桌面走最高档（Hi-Res 随会员权益）、蜂窝限 192K、弱网降 64K 后 10 分钟窗口自然回升。
- **队列稳定（A1）**：playurl 多候选节点按序换源重试（首字节前），CDN 单点故障不再 502，回归覆盖。
- **播放页档位小字**：实际音质（Hi-Res/192K/杜比…）显示在进度条下方居中；镜像态不显示本机探测值。账号页写死的「音质最高 192K」文案移除。
- **收藏星身份归一**：曲库索引按 bvid 注册别名——同一首歌从 UP 主页（试听流）与从歌单（曲库行）进来星状态一致；试听态点星取消收藏不再走重导入兜底。
- **队列滑动窗口**：三胶囊（随机/循环/∞）与「播放队列」表头固定，滚动只发生在列表窗口内，上/下边缘 mask 渐隐软收，不硬截断。
- **曲库正在播放标记**：当前播放行序号位显示粉色音谱动画（暂停时冻结），打开详情页/列表重渲染后自动补打标记。
- **Windows 滚轮 ↔ 横排**：悬停在专辑架 / UP 热门 / 风格胶囊等横排上时垂直滚轮平移为横向滚动，滚到头自然交给页面。
- 版本号：android 2.1.0(14)、desktop 2.1.0
- 回归：pytest 219 过（1 项失败为 Windows 缺 `os.pwrite` 的既有平台限制，与本版无关）/ node --test 53

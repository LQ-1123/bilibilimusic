...
| 粤语逐句学习 | 在歌词页点「学」，显示自动粤拼；点单字看本句读音和声调并听单字朗读，也可录音回听、单句循环、标记掌握（进度保存在本机） |
| 录歌 | 歌词页点「唱」，将当前播放的伴奏声音与所选麦克风混合，录成可试听、下载的音频；默认使用系统输入设备 |
...
学粤语时，搜索歌名并播放有歌词的版本，打开歌词页，点击「学」进入逐句学习。点击歌词可选句并跳转到对应时间；点击歌词中的单字会显示该字在本句中的粤拼和声调，并朗读单字，也可点「再听」。无时间轴的歌词仍可学习粤拼和跟读，但不支持单句循环。粤拼由内置的 [ToJyutping](https://github.com/CanCLID/ToJyutping) 自动标注，个别多音字建议结合原唱核对；浏览器朗读优先使用本地粤语声线，并在打开学习模式时预热。若只有在线声线，首次启动仍可能受网络影响；录音需要麦克风权限。

录歌：先在 BiliMusic 播放想唱的伴奏版本并打开歌词页，点击与「学」并列的「唱」。默认使用系统输入设备，也可从下拉框选择麦克风；首次使用如看不到设备名称，可点「刷新设备」授予权限。点「开始录歌」后，当前歌曲声音和麦克风人声会混成一条音轨；唱完点「结束」，即可试听并下载录音，不录视频画面。建议戴耳机；录制在浏览器本机完成，不上传文件，离开页面前请下载。此功能使用浏览器的媒体捕获、Web Audio 混音和 MediaRecorder，需在 HTTPS 或本机地址使用新版 Chrome / Edge。
...
备份时保留 `data/`；不要提交其中的数据库或 Cookie。当前版本以在线播放为主，不提供离线音乐库；但最近**听过的歌会自动留一份轻缓存**（池上限 500MB，超出自动淘汰最久未听的，账号页可一键清空）：网络不稳或断网时，听过的部分照常回放，不打断听歌。旧版本本地音频会在启动迁移时尝试转为在线记录，迁移成功后移除旧文件。
